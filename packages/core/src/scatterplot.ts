...
function generateMessages(
  ...
  if (spec.legendTitle?.value !== "") {
    messages.unshift({
      ...
      text: `The color of the scatterplot elements represents the amount of <i>${
        spec.legendTitle?.value
      }  </i>, where ${createColorRect(
        spec.minColor?.value
      )} represents the lowest and ${createColorRect(
        spec.maxColor?.value
