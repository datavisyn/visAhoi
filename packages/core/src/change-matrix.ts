...
function generateMessages(
  ...
  modebarInteractions.set(
    ...
    `${
      modebarText.includes("Download plot as a png")
        ? `${SvgIcons.CAMERA} <b>Screenshot</b>: You can download a .png of the change-matrix.<br/><br/>`
        : ""
    }`
  );
  ...
  const messages: IOnboardingMessage[] = [
    ...
    },
    {
      ...
      text: `The legend shows the <i>${
        spec.chartTitle?.value
      }</i> where the ${createColorRect(
        spec.minColor?.value
      )} means low and ${createColorRect(spec.maxColor?.value)} means high.`,
      title: "Reading the chart",
      onboardingStage: reading,
      marker: {
