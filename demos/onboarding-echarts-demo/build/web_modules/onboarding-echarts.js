...
function createAnchor(d, i, nodes) {
    ...
    const parentEl = select(currentEl).node().parentNode;
    const hintGroup = select(parentEl)
...
function generateChartAnchors(anchors) {
    ...
    anchors.forEach(el => {
        ...
        if (a.coords) {
            ...
            const elToAppendTo = select(a.sel);
...
function createHint(settings, text) {
    ...
    const hG = select('.onboardingAnnotations')
        .append('g')
...
function displayOnboardingMessages(messages) {
    const onboardingLegend = select('#onboarding')
    ...
    select('svg').append('g').classed('onboardingAnnotations', true);
...
function generateOnboardingSpec(chart, coords) {
...
function barChartFactory(chart, coords) {
...
function generateOnboardingSpec$1(chart, coords) {
    // const dataCoords = chart._chartsViews[0]._data._itemLayouts;
...
function changeMatrixFactory(chart, coords) {
    const onbordingSpec = generateOnboardingSpec$1(chart, coords);
    return generateOnboardingMessages$3(EChartType.CHANGE_MATRIX, onbordingSpec);
}

function generateOnboardingSpec$2(chart, coords) {
    const dataCoords = chart._chartsViews[0]._data._itemLayouts;
...
function horizonGraphFactory(chart, coords) {
    const onbordingSpec = generateOnboardingSpec$2(chart, coords);
    return generateOnboardingMessages$3(EChartType.HORIZON_GRAPH, onbordingSpec);
}

async function onboarding(chartType, chart) {
    console.log("---> ", chart);
    const coords = {};
