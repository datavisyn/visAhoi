...
var EChartType;
(function (EChartType) {
    EChartType["BAR_CHART"] = "bar-chart";
    EChartType["CHANGE_MATRIX"] = "change-matrix";
    EChartType["HORIZON_GRAPH"] = "horizon-graph";
...
function generateOnboardingMessages$1(spec) {
    const messages = [
        {
            anchor: spec.chartTitle?.anchor,
            requires: ['chartTitle'],
...
const changeMatrix = {
    generateOnboardingMessages: generateOnboardingMessages$1
};

function createColorRect(color = 'white') {
    return `<div class="colorRect" style="background: ${color}"></div>`;
}
function generateOnboardingMessages$2(spec) {
    const messages = [
        {
            anchor: spec.chartTitle?.anchor,
            requires: ['chartTitle'],
...
const horizonGraph = {
    generateOnboardingMessages: generateOnboardingMessages$2
};
 ...
 */
function createAnchor(d, i, nodes) {
    const currentEl = nodes[i];
    const parentEl = select$1(currentEl).node().parentNode;
    const hintGroup = select$1(parentEl)
...
function generateChartAnchors(anchors) {
    ...
    anchors.forEach(el => {
        ...
        if (a.coords) {
            ...
            const elToAppendTo = select$1(a.sel);
...
function createHint(settings, text) {
    ...
    const hG = select$1('.onboardingAnnotations')
        .append('g')
...
function displayOnboardingMessages(messages) {
    const onboardingLegend = select$1('#onboarding')
    ...
    select$1('svg').append('g').classed('onboardingAnnotations', true);
...
function generateOnboardingMessages$3(chartType, spec) {
    switch (chartType) {
        case EChartType.BAR_CHART:
            return barChart.generateOnboardingMessages(spec);
        case EChartType.CHANGE_MATRIX:
            return changeMatrix.generateOnboardingMessages(spec);
        case EChartType.HORIZON_GRAPH:
            return horizonGraph.generateOnboardingMessages(spec);
    }
    return [];
}
...
function generateOnboardingSpec(vegaSpec, aggregatedValues, elems) {
    const v = vegaSpec;
...
function barChartFactory(vegaSpec, aggregatedValues, elems) {
    const onbordingSpec = generateOnboardingSpec(vegaSpec, aggregatedValues, elems);
    // console.log('Generated Spec: ', onbordingSpec);
    return generateOnboardingMessages$3(EChartType.BAR_CHART, onbordingSpec);
}

function generateOnboardingSpec$1(vegaSpec, elems) {
    const v = vegaSpec;
    return {
        chartTitle: {
            value: (typeof (v.title) === 'string') ? v.title : v.title?.text,
...
function changeMatrixFactory(vegaSpec, elems) {
    const onbordingSpec = generateOnboardingSpec$1(vegaSpec, elems);
    // console.log('Generated Spec: ', onbordingSpec);
    return generateOnboardingMessages$3(EChartType.CHANGE_MATRIX, onbordingSpec);
}

function generateOnboardingSpec$2(vegaSpec, visualizationSpec, elems) {
    const v = vegaSpec;
...
function horizonGraphFactory(vegaSpec, visualizationSpec, elems) {
    const onbordingSpec = generateOnboardingSpec$2(vegaSpec, visualizationSpec, elems);
    // console.log('Generated Spec: ', onbordingSpec);
    return generateOnboardingMessages$3(EChartType.HORIZON_GRAPH, onbordingSpec);
}

async function onboarding(chartType, vegaResult) {
    const evaluated = await vegaResult.view.evaluate(); // TODO: `evaluate()` is not an officially supported Vega API
    ...
    const nodes = select('#vis').selectAll('.role-mark').selectAll('path').nodes();
    // Get the data of the individual bars
    const d3Data = nodes.map((el) => el.__data__);
